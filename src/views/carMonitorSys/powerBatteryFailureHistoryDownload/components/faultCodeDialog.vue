...
// request
import { getFaultCode } from "@/api/carMonitorSys/powerBatteryFailureHistoryDownload";
export default {
	name: "faultCodeDialog",
	...
	methods: {
		...
		listLoad() {
			...
			getFaultCode(this.listQuery).then(({ data }) => {
				if (data.code === 0) {
					this.list = data.data;
