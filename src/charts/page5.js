...
// 左二圆滑折线图
function chargingLineChartTwo(nameList, colors, xData, yData1) {
  let optionData = {
    // backgroundColor: "rgba(30, 34, 48, 1)",
    grid: {
      top: "19%",
...
// 右一折线图
function chargingBarChartOne(nameList, colors, xData, yData1) {
  let optionData = {
    // backgroundColor: "rgba(30, 34, 48, 1)",
    grid: {
      top: "19%",
        ...
        for (let i = 0; i < params.length; i++) {
          ...
          if (params[i].seriesName == "换电时长") {
            returnData +=
              ...
              '</span><span style="display:inline-block; margin-left: 4px; line-height: 10px; font-family: MicrosoftYaHei; font-size: 12px; color: #fff' +
              '">小时</span><br/>';
          } else if (params[i].seriesName == "换电次数") {
            returnData +=
              '<span style="font-family: MicrosoftYaHei; font-size: 12px; color: #fff' +
              '">' +
              params[i].seriesName +
