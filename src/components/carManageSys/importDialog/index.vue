<template>
	...
	>
		<div slot="formContent">
			...
			<!-- <div class="import-remark">
				<span class="textColor">注：</span>若已上传过的文件需重新选择方可上传，最多导入<span class="textColor"> {{ maxNumber }} </span>行
			</div> -->
			<!-- 导入失败信息插槽 -->
			<slot />
	...
	methods: {
      ...
      //       this.closeDialog();
		...
		fileSuccess(response) {
			if (response.code === 0 && (response.data || this.isData)) {
				this.$notify({
					title: "成功",
					message: "文件上传成功",
					type: "success",
