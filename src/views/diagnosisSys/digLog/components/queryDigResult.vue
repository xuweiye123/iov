    ...
    <div slot="drawerContent">
      ...
      <div class="">
        ...
        <el-table
          ...
          v-show="reportVisible"
        >
          <el-table-column prop="content" label="故障描述">
            <template slot-scope="scope">
              <span>{{
                scope.row.content ? scope.row.content.split(";")[0] : ""
              }}</span
              >&nbsp;&nbsp;&nbsp;&nbsp;
              <span
                v-if="scope.row.content && scope.row.content.split(';')[1]"
                >{{ scope.row.content.split(";")[1] }}</span
              >
            </template>
          </el-table-column>
          <el-table-column prop="result" width="350" label="故障码">
            <template slot-scope="scope">
              <span>{{
...
export default {
  name: "queryDigResult",
  doNotInit: true,
  mixins: [pagingMixin, tableStyle, drawerOtherHeight, commonExport],
  computed: {
    filterTableList() {
      if (this.digType === "12") {
  ...
  methods: {
    ...
    listLoad() {
      ...
      this.exportData = {};
      if (this.digType == 7) {
        getDxResultDataStream({ token: this.digServiceId })
          .then(({ data }) => {
            if (data.code === 0) {
              let d = [];
              if (data.data.result) {
        ...
        getDxResult({ token: this.digServiceId })
          .then(({ data }) => {
            if (data.code === 0) {
              let d = [];
              if (data.data.result) {
