...
export default {
		...
		return {
			...
			events: {
				init() {
					...
					self.map.setFitView([self.polyline]);
				},
			},
		};
	},
			...
			if (e1) {
				...
				getWebSocket().then(({ data }) => {
					if (data.code === 0) {
						...
						localStorage.setItem("socketAddress", urlBefore + data.data);
						return data;
					} else {
	...
	methods: {
		...
		_webSocket(e) {
			...
			if (isDriving) {
				// 行驶状态才会出现轨迹
				this.lineArr.push(new AMap.LngLat(lat, lng));
					...
					for (var j = 0; j < titleArry.length; j++) {
						...
						for (var k = 0; k < listArry.length; k++) {
							if (titleArry[j] === listArry[k].moduleName) {
								...
								let reg = /^(SOC|电池SOC"%"|动力电池剩余电量SOC|动力电池剩余电量SOC"%"|动力电池剩余电量SOC_C200|动力电池真实SOC|蓄电池荷电状态（SOC）|车辆电池电量SOC|SOC"%"|动力电池SOC|动力电池SOC"%"|电池SOC|SOC电量|SOC电量(%))$/;
