    ...
    loadMoreVINFilter(value) {
      if(this.isImport){
        ...
        this.vinListArr=this.vinListArr.filter(i=>{
           return i.vinNoTotal.indexOf(value) != -1
        })
        ...
        //  let dataList = res.data.data?res.data.data.map((item)=>{return item.vinNo}):[];
         let dataList = res.data.data?res.data.data:[];
          ...
          if(res.data.data&&res.data.data.length>0){
            ...
            console.log(this.vinListArr,"this.vinListArr")
          }else{
            this.vinListArr =[]
          }
        }else{
